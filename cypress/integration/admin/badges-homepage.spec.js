...
describe('Badges homepage table view', () => {
    ...
    it('should be able to search badges', () => {
        fetchBadges()
            .then((badges) => {
                ...
                cy.get('.submenu-section .badges-search .search-input')
                    .type(selectedBadge.description.substr(0, 30));

                //select all on current view
                cy.get('table.badges-table tbody tr td')
                    .contains(selectedBadge.name);
            })
    ...
    it('should be able to change badges displayed per table page', () => {
        ...
        cy.get('.footer-pagination #perPage')
            .select('20');

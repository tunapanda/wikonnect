...
// Words from http://www2.imm.dtu.dk/pubdb/views/publication_details.php?id=6010
// JSON from https://github.com/wooorm/afinn-111/blob/master/data/afinn-111.json
let words = {
  "abandon": -2,
  "abandoned": -2,
  "abducted": -2,
...
export default Service.extend({
  ...
  sentiment: function(string) {
    ...
    let wordArray = this._stripNonWords(string).split(' ');
  ...
  detailedSentiment: function(string) {
    let words = this.words;
    let wordArray = this._stripNonWords(string).split(' ');
    let result = {positive: 0, negative: 0, count:0, posPercent: 0, negPercent: 0};
    wordArray.forEach(function(word) {
      result.count += 1;
      if (words[word] !== undefined) {
        let valence = words[word];
        if (valence > 0) {
          result.positive += 1;
        } else if (valence < 0) {
          result.negative += 1;
